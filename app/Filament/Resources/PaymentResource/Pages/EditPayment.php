...
class EditPayment extends EditRecord
{
    ...
    protected function mutateFormDataBeforeFill(array $data): array
    {
        ...
        \Log::info('Cargando archivos existentes Payment:', [
    ...
    {
        \Log::info('=== INICIO AfterSave Payment Debug ===');
        ...
        if (!empty($attachmentFiles)) {
            \Log::info('Procesando archivos Payment en edición...');
            ...
            foreach ($attachmentFiles as $index => $file) {
                ...
                if (is_object($file) && method_exists($file, 'getClientOriginalName')) {
                    \Log::info('Archivo Payment nuevo válido, procesando...');
                    ...
                    try {
                        ...
                        \Log::info('Información del archivo Payment nuevo:', [
                        ...
                        \Log::info('Archivo Payment nuevo almacenado:', [
                            'file_name' => $fileName,
                            'file_path' => $filePath
                        ]);
                        ...
                        \Log::info('Attachment Payment nuevo creado exitosamente:', [
                            'attachment_id' => $attachment->id,
                            'attachment' => $attachment->toArray()
                        ]);
                        
                    } catch (\Exception $e) {
                        \Log::error('Error al procesar archivo Payment nuevo:', [
                            'error' => $e->getMessage(),
                            'trace' => $e->getTraceAsString()
                        ]);
                    }
                } else {
                    \Log::info('Archivo Payment existente (string) o no válido:', [
                        'file' => $file,
            ...
            \Log::info('No se encontraron archivos para procesar en Payment (edición)');
        }
        
        // Verificar datos finales
        $finalAttachments = $this->record->attachments();
        \Log::info('Attachments asociados al Payment (después de edición):', [
            'count' => $finalAttachments->count(),
            'attachments' => $finalAttachments->get()->toArray()
        ]);
        
        \Log::info('=== FIN AfterSave Payment Debug ===');
    }
}