...
class AttachmentsRelationManager extends RelationManager
{
    ...
    protected static ?string $title = '📎 Documentos del Pago';
        ...
        return $table
            ...
            ->recordActions([
                Action::make('download')
                    ...
                    ->openUrlInNewTab()
                    ->tooltip('Descargar archivo del pago'),
                    
                DeleteAction::make()
                    ->label('🗑️ Eliminar')
                    ->color('danger')
                    ->requiresConfirmation()
                    ->modalHeading('⚠️ Eliminar archivo del pago')
                    ->modalDescription('¿Estás seguro de que quieres eliminar este archivo del pago? Esta acción no se puede deshacer.')
                    ->modalSubmitActionLabel('Sí, eliminar archivo')
                    ->modalCancelActionLabel('Cancelar')
                    ->successNotificationTitle('Archivo eliminado correctamente'),
            ])
            ->bulkActions([
                // Bulk actions disabled for simplicity following KISS principle
            ])
            ->defaultSort('created_at', 'desc')
            ->striped()
            ->emptyStateHeading('📎 No hay documentos adjuntos')
            ->emptyStateDescription('Los archivos subidos en el formulario del pago aparecerán aquí.')
