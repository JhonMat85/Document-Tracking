...
class CreateHes extends CreateRecord
{
    ...
    {
        \Log::info('=== INICIO AfterCreate HES Debug ===');
        ...
        if (!empty($attachmentFiles)) {
            \Log::info('Procesando archivos HES...');
            ...
            foreach ($attachmentFiles as $index => $file) {
                ...
                if (is_object($file) && method_exists($file, 'getClientOriginalName')) {
                    \Log::info('Archivo HES válido, procesando...');
                    ...
                    try {
                        ...
                        \Log::info('Información del archivo HES:', [
                        ...
                        \Log::info('Archivo HES almacenado:', [
                            'file_name' => $fileName,
                            'file_path' => $filePath
                        ]);
                        ...
                        \Log::info('Attachment HES creado exitosamente:', [
                            'attachment_id' => $attachment->id,
                            'attachment' => $attachment->toArray()
                        ]);
                        
                    } catch (\Exception $e) {
                        \Log::error('Error al procesar archivo HES:', [
                            'error' => $e->getMessage(),
                            'trace' => $e->getTraceAsString()
                        ]);
                    }
                } else {
                    \Log::warning('Archivo HES no válido:', [
                        'file' => $file,
                        'is_object' => is_object($file),
            ...
            \Log::info('No se encontraron archivos para procesar en HES');
        }
        
        // Verificar datos finales
        $finalAttachments = $this->record->attachments();
        \Log::info('Attachments asociados al HES:', [
            'count' => $finalAttachments->count(),
            'attachments' => $finalAttachments->get()->toArray()
        ]);
        
        \Log::info('=== FIN AfterCreate HES Debug ===');
    }
