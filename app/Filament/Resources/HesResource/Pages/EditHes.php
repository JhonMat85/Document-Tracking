...
class EditHes extends EditRecord
{
    ...
    protected function mutateFormDataBeforeFill(array $data): array
    {
        ...
        \Log::info('Cargando archivos existentes HES:', [
            'hes_id' => $this->record->id,
    ...
    {
        \Log::info('=== INICIO AfterSave HES Debug ===');
        
        // Log del estado del formulario
        $rawState = $this->form->getRawState();
        \Log::info('Form Raw State HES completo:', $rawState);
        
        // Manejar los archivos subidos
        $attachmentFiles = $rawState['attachment_files'] ?? [];
        
        \Log::info('Attachment Files HES encontrados:', [
        ...
        if (!empty($attachmentFiles)) {
            \Log::info('Procesando archivos HES en edición...');
            
            foreach ($attachmentFiles as $index => $file) {
                \Log::info("Archivo HES #{$index}:", [
                    'file' => $file,
                ...
                if (is_object($file) && method_exists($file, 'getClientOriginalName')) {
                    \Log::info('Archivo HES nuevo válido, procesando...');
                    ...
                    try {
                        ...
                        \Log::info('Información del archivo HES nuevo:', [
                        ...
                        $filePath = $file->storeAs('hes-attachments', $fileName, 'public');
                        
                        \Log::info('Archivo HES nuevo almacenado:', [
                            'file_name' => $fileName,
                            'file_path' => $filePath
                        ]);
                        ...
                        $attachment = Attachment::create([
                            'attachable_type' => 'App\\Models\\Hes',
                        ...
                        \Log::info('Attachment HES nuevo creado exitosamente:', [
                            'attachment_id' => $attachment->id,
                            'attachment' => $attachment->toArray()
                        ]);
                        
                    } catch (\Exception $e) {
                        \Log::error('Error al procesar archivo HES nuevo:', [
                            'error' => $e->getMessage(),
                            'trace' => $e->getTraceAsString()
                        ]);
                    }
                } else {
                    \Log::info('Archivo HES existente (string) o no válido:', [
                        'file' => $file,
            ...
            \Log::info('No se encontraron archivos para procesar en HES (edición)');
        }
        
        // Verificar datos finales
        $finalAttachments = $this->record->attachments();
        \Log::info('Attachments asociados al HES (después de edición):', [
            'count' => $finalAttachments->count(),
            'attachments' => $finalAttachments->get()->toArray()
        ]);
        
        \Log::info('=== FIN AfterSave HES Debug ===');
    }
}